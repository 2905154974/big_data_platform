...
//    }
     ...
     *
     * */
    public static List<big_data_phone> excelToPhoneInfoList(InputStream inputStream) {
        List<big_data_phone> list = new ArrayList<>();
        Workbook workbook = null;
        try {
            workbook = WorkbookFactory.create(inputStream);
            inputStream.close();
            ...
            Cell cell = row.getCell(0);
            for (int i = 1; i <= rowLength; i++) {
                big_data_phone goodInfo = new big_data_phone();
                row = sheet.getRow(i);
                for (int j = 0; j < colLength; j++) {
                    cell = row.getCell(j);
                    if (cell != null) {
                        ...
                        if (j == 0) {
                            goodInfo.setAdminId(GlobalData.ADMIN_ID);//属于那个商家
                            goodInfo.setName(data);
                        } else if (j == 1) {
                            goodInfo.setPhone(data);
                        } else if (j == 2) {
                            goodInfo.setSex(data);
                        }
                    }
                }
                list.add(goodInfo);
